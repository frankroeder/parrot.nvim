...
describe("Groq", function()
  ...
  describe("check", function()
    it("should return true for supported models", function()
      assert.is_true(groq:check({ model = "llama-3.1-70b-versatile" }))
      assert.is_true(groq:check({ model = "mixtral-8x7b-32768" }))
    end)

    it("should return nil for unsupported models", function()
      assert.is_nil(groq:check({ model = "unsupported-model" }))
    end)
  end)
end)
